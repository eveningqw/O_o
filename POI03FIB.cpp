...
int main(){
  ...
  fst[2] = 'a';
  for(int i = 3; i <= n; ++i){
    if((int)lst[i - 2].size() >= sz){
      lst[i] = lst[i - 2];
    } else{
      lst[i] = lst[i - 1] + lst[i - 2];
      while((int)lst[i].size() > sz){
        lst[i].erase(lst[i].begin());
      }
    }
  }
  for(int i = 3; i <= n; ++i){
    if((int)fst[i - 1].size() >= sz){
      fst[i] = fst[i - 1];
    } else{
      fst[i] = fst[i - 1] + fst[i - 2];
      while((int)fst[i].size() > sz){
        fst[i].pop_back();
      }
    }
  }
  for(int i = 3; i <= n; ++i){
    ...
    for(int l1 = 1; l1 < sz; ++l1){
      string fr;
      int bad = 0;
      for(int j = (int)lst[i - 1].size() - l1; j < (int)lst[i - 1].size(); ++j){
        if(j < 0){
          bad = 1;
          break;
        }
      ...
      for(int j = 0; j < sz - l1; ++j){
        if(j >= (int)fst[i - 2].size()){
          bad = 1;
          break;
        }
        fr += fst[i - 2][j];
